...
function tbMagic:TableConcat(t1,t2)
...
function tbMagic:MagicEnter(IDs, IsThing)
	...
	for i=0,count - 1,1 do
		...
		if npc.Camp == CS.XiaWorld.Fight.g_emFightCamp.Player and  npc.IsDisciple and npc.Sex ~=self.bind.Sex then
			--內門
			table.insert(list1,npc)
		elseif npc.Camp == CS.XiaWorld.Fight.g_emFightCamp.Player and npc.IsDisciple == false and npc.Sex ~=self.bind.Sex then
			--外門
			table.insert(list2,npc)
		elseif npc.Camp == CS.XiaWorld.Fight.g_emFightCamp.Friend and npc.Sex ~=self.bind.Sex then
			--友軍
			table.insert(list3,npc)
		end
	end
	hglist = self:TableConcat(hglist, list1)
	hglist = self:TableConcat(hglist, list2)
	hglist = self:TableConcat(hglist, list3)
	self:EnterChoice(hglist)
end

function tbMagic:MagicStep(dt, duration)
	if jump >= 1 and flag == 1 then
		local count = #nowlist
		print(i)
		if i > count then
			return 1;	
...
function tbMagic:Action(player,target)
	...
	local random_pic = player.LuaHelper:RandomInt(214,320);
	local img = "" --"[img]".."Spr/preview_icon/"..random_pic..".png[/img]\n"
	local desc = img
	...
	if zCD == 0 then --双方双修CD判定
		...
		local shouyuan = player.LuaHelper:GetProperty("MaxAge") - 60
			if shouyuan > 0 then
			desc = img..target.Name.."正在结丹的关键时刻，"..player.Name.."提议帮对方补补魔，"..target.Name.."闻之大喜任"..player.Name.."在自己身上驰骋……\n事毕，"..target.Name.."吸收了"..player.Name.."注入自己体内的精元，化为了灵气……\n此次双修"..player.Name.."损耗了一甲子的寿元……"
			...
			desc = img..player.Name.."已经不足一甲子了，想来就算用命去换，也对"..target.Name.."结丹帮助不大了……"
			end
		return desc;
		end
		...
		desc = img..player.Name.."对"..target.Name.."提出了双修的请求，然而"..target.Name.."是一个贞洁的女子，她拒绝了"..player.Name.."。"
		return desc;
		end
		if player.LuaHelper:GetModifierStack("Zhenchaosuo")~= 0 or player.LuaHelper:GetModifierStack("Zhenchaozhou") ~= 0 then--1带着贞操锁/咒
			...
			desc = img..target.Name.."想要侍奉自己的主人，许是"..player.Name.."心情很好，她解开了贞操锁/咒，让"..player.Name.."用胯下肉棒侍奉了自己"
			tbTalkAction:xuexi(player,target)
				...
				desc = img..target.Name.."想要侍奉自己的主人，许是"..player.Name.."心情很好，他解开了贞操锁/咒，让"..player.Name.."使用小穴侍奉了自己"
				tbTalkAction:xuexi(player,target)
				else
					desc = img..player.Name.."的下体被阳锁/贞操咒封住了，并不能对对方做什么，于是转身离开了。"
				end
			end
		return desc;
		end
		if target.LuaHelper:GetModifierStack("Zhenchaosuo")~= 0 or target.LuaHelper:GetModifierStack("Zhenchaozhou") ~= 0 then--2带着贞操锁/咒
			...
			desc = img..player.Name.."一时性起，想要干自己的女奴"..target.Name.."一炮，他解开了贞操锁/咒，在"..target.Name.."身上耕耘许久直到心满意足后转身离去了。"
			tbTalkAction:xuexi(player,target)
			else
				...
				desc = img..player.Name.."一时性起，感觉下身湿润了，想要自己的公狗"..target.Name.."来干上自己一炮，他解开了贞操锁/咒，"..target.Name.."用胯下肉棒侍奉了"..player.Name.."。"
				tbTalkAction:xuexi(player,target)
				else
					if player.PropertyMgr.RelationData:IsRelationShipWith("Spouse",target) == true then--是夫妻，离婚
					desc = img..player.Name.."开口向"..target.Name.."要求双修，"..target.Name.."百般推脱不成，反倒激怒了"..player.Name.."一把揽住"..target.Name.."便伸手往下探去。直到触及一硬物……\n只闻"..player.Name.."留下了一句“贱人”……\n"..player.Name.."与"..target.Name.."解除了婚姻关系。"
					player.PropertyMgr.RelationData:RemoveRelationShip(target,"Spouse");
					else--带锁拒绝
					desc = img..target.Name.."因为下身被锁，故拒绝了双修。"
					end
				end
			end
		return desc;
		...
		if player.Sex == CS.XiaWorld.g_emNpcSex.Male then--性别判定
			...
			desc = img..player.Name.."向"..target.Name.."提出双修的邀请，"..target.Name.."作为妻子，自然不会拒绝"..player.Name.."的要求，于是双方进行了一次和谐的双修。"
			tbTalkAction:xuexi(player,target)
			else
				if player.PropertyMgr.RelationData:IsRelationShipWith("Luding2",target) == true then--是否主人
				desc = "对于主人的需求，身为母狗的"..target.Name.."自然是万万不敢拒绝的，于是"..target.Name.."跪伏下身子开始为她的主人服务……\n这是一次双方都很满意的双修。"
				tbTalkAction:xuexi(player,target)
				else
					if target.PropertyMgr:CheckFeature("Pogua") ~= true then--对方是处女
						if Dfxg == CS.XiaWorld.g_emJHNpc_Feature.Tenacity then--坚毅性格，只愿意被比自己强且是金丹以上的大佬破处
							...
							desc = img..player.Name.."向"..target.Name.."提出双修的邀请，"..target.Name.."本想拒绝，可对方那强悍的修为着实让"..target.Name.."心动不已，只听"..target.Name.."呢喃一句：“为了成就大道，这处女之身，弃了便弃了罢”。\n"..player.Name.."闻言大喜，褪下裤子露出胯下巨物，便与对方缠绵了起来……\n一番云雨过后，看着身旁滴滴落红，"..player.Name.."兴致又起再次拖着"..target.Name.."酣战了数番。"
							tbTalkAction:xuexi(player,target)
							else
							desc = img..target.Name.."表示自己一心修行，无心想着那些旁门左道的双修之法，所以并不愿意和"..player.Name.."双修。"
							end
						else
							if Dfxg == CS.XiaWorld.g_emJHNpc_Feature.Greedy then--贪婪性格，会被身价百万的男性草服，成为对方母狗，也原意被拥有10W以上的男修破处
								...
								tbTalkAction:xuexi(player,target)
									...
									player:AddModifier("Diyidixue");
									player.PropertyMgr.RelationData:AddRelationShip(target,"Chunv");
									desc = img..player.Name.."向"..target.Name.."提出双修的邀请，没想到这种富甲一方的男人会愿意给自己开苞，不禁下身湿透意乱情迷……\n许是那财富的光环迷人眼眸，"..target.Name.."不但没有感受到破瓜之痛，反而越挨炮越来劲，口中淫言浪语不断……\n事毕，"..target.Name.."的蜜穴里还插着"..player.Name.."的肉棒，两人均非常满足的拥在了一起……"
									tbTalkAction:xuexi(player,target)
									else
									desc = img..target.Name.."嫖了一眼"..player.Name.."后，清冷的吐出了一个“滚”字。"
									end
								end
							else
						...
						if player.LuaHelper:GetGLevel() > target.LuaHelper:GetGLevel() then--男性主角，且比对象强
							...
							desc = img..player.Name.."向"..target.Name.."提出双修的邀请，"..target.Name.."显然不太擅长拒绝强者的要求，毕竟在修真界与强者双修本身就是一件挺划得来的事情，对觊觎仙途的修真者们来说，所谓贞洁大不抵只是个笑话罢了\n"..target.Name.."褪下衣衫后便与"..player.Name.."搂作一团翻云覆雨了起来。\n这是一次不错的双修，双方都获得了满足。"
							else
								if random > 20 and tmeili > 8 then--对方魅力大于8有20%概率发生以下事件
								player.PropertyMgr:AddMaxAge(10);
								target.PropertyMgr:AddMaxAge(30);
								desc = img..player.Name.."向"..target.Name.."提出双修的邀请，"..target.Name.."却显得不太情愿，可"..target.Name.."这等绝世容颜显然让"..player.Name.."欲火焚身难以自持了\n只见"..player.Name.."三下五除二的褪下对方衣衫，信手掏出胯下之物便如急色鬼附身一样在"..target.Name.."身上奋战了起来，"..player.Name.."的肉棒于于"..target.Name.."的蜜穴中来回冲杀，战至酣时"..player.Name.."甚至忘记了双修的本意，也管不得什么交而不泄止泄固元了。\n随着一声怒吼声，"..player.Name.."将自己滚热的阳精全部灌注于"..target.Name.."的蜜穴里。\n虽然这次双修最后还是成功了，但是这次精关失守"..player.Name.."损失了近一甲子的寿元。"
								else
									if random > 10 then--如果没有发生上面的事件，那有30%触发此事件
									desc = img..player.Name.."向"..target.Name.."提出双修的邀请，"..target.Name.."却显得不太情愿，然而实力的差距容不得"..target.Name.."摇头说不，只见"..player.Name.."施术摄住"..target.Name.."后便掏出肉棒不管不顾的捅进"..target.Name.."小穴之中，一阵阵猛烈的活塞运动让尚未做好准备的"..target.Name.."被干的两眼翻白几度昏死过去后又被操醒过来……\n酣畅之后，心满意足的"..player.Name.."放过了"..target.Name.."。"
									else--10%概率发生以下事件
									player.PropertyMgr.RelationData:AddRelationShip(target,"Luding2");
									desc = img..player.Name.."刚向"..target.Name.."提出双修的邀请，"..target.Name.."就好像显得特别高兴并露出迫不及待的样子，还没等"..player.Name.."准备做些什么，"..target.Name.."便已经伸手掏出了"..player.Name.."的肉棒，一口猛的吞了进去，"..player.Name.."只觉下体被那温暖的小口含住……\n事毕，"..target.Name.."用着香舌帮着"..player.Name.."清理肉棒上的污渍，还满脸淫贱相的说着什么“主人在干我一次吧”之类的话语。"
									end
								end
							end
							tbTalkAction:xuexi(player,target)
							...
							if player.LuaHelper:GetGLevel() < target.LuaHelper:GetGLevel() then--男主，没女目标实力强
								...
								player.PropertyMgr.RelationData:AddRelationShip(target,"Luding2");
								desc = img..player.Name.."向"..target.Name.."提出双修，虽然这是一个看实力的修真世界，但是偶尔英俊的脸庞还是能起一些作用的，比如眼前的女修"..target.Name.."便是轻点足尖仿佛等待着些什么，"..player.Name.."仿佛领悟到了，伏下身子捧起对方的玉足，伸出舌头在"..target.Name.."玉足的每一寸肌肤上均舔舐了数个来回……\n精心的侍奉让"..target.Name.."感到愉悦，心情大好的"..target.Name.."甚至主动跨坐于"..player.Name.."腿上摇摆不止。\n对"..player.Name.."来说这是一次成功的双修，而对"..target.Name.."来说，则是获得了一条公狗，但至少这是各取所需不是么。"
								tbTalkAction:xuexi(player,target)
								else
								desc = img..player.Name.."刚向"..target.Name.."提出双修的邀请，之听"..target.Name.."口中冷清冷的吐出了一个“滚”字，便灰溜溜的离开了。"
								end
							else--男主，和女目标境界相同
							desc = img..player.Name.."的境界与"..target.Name.."相差无几，开口提出双修对方也没有拒绝。\n这是一次成功的双修。"
							tbTalkAction:xuexi(player,target)
							end
						end
					end
			...
			desc = img..player.Name.."向"..target.Name.."提出双修的邀请，"..target.Name.."作为丈夫，自然不会拒绝"..player.Name.."的要求，于是双方进行了一次和谐的双修。"
			tbTalkAction:xuexi(player,target)
			else
				if player.PropertyMgr.RelationData:IsRelationShipWith("Luding2",target) == true then--女主，主仆
				desc = img..target.Name.."一听到自己的主人邀请自己双修，激动地跪倒在地匍匐在"..player.Name.."脚边做摇尾乞怜状，"..player.Name.."大喜，恩赐似的跨坐于"..target.Name.."脸上道：“乖狗狗，来舔”……\n随即双方进行了一次和谐的双修。"
				tbTalkAction:xuexi(player,target)
				...
				desc = "没有任何男人可能拒绝的了"..player.Name.."，"..player.Name.."对"..target.Name.."提出了双修的要求，"..target.Name.."大喜之余化身为最卑微的公狗，四肢着地的舔舐起了"..player.Name.."玉足。\n在"..player.Name.."的面前，"..target.Name.."没有丝毫尊严的倾服于"..player.Name.."，经过"..target.Name.."努力且卑微的服侍之后，"..player.Name.."同意了"..target.Name.."成为自己的狗。\n这是一场双方都很满意的双修。"
				tbTalkAction:xuexi(player,target)
				else
					...
					tbTalkAction:xuexi(player,target)
						...
						if Dfxg == CS.XiaWorld.g_emJHNpc_Feature.Tenacity then--女主，对方坚毅人格
							...
							if Dfxg == CS.XiaWorld.g_emJHNpc_Feature.Greedy or Dfxg == CS.XiaWorld.g_emJHNpc_Feature.Naive or Dfxg == CS.XiaWorld.g_emJHNpc_Feature.Weak then--女主，对方贪婪天真或者软弱人格
								...
								player.PropertyMgr.RelationData:AddRelationShip(target,"Luding2");
								desc = img..player.Name.."向"..target.Name.."提出双修，"..target.Name.."甚至完全没想到会有此等好事发生在自己身上，居然有实力高强的女修士愿意给自己送干还外带修炼的，只要脑子没坏自是不会错过这等好事。\n这是一次成功的双修，双修结束后"..target.Name.."还久久不愿离去，腆着脸要给"..player.Name.."当狗。"
								else
									if player.LuaHelper:GetGLevel() < target.LuaHelper:GetGLevel() then--女主，没男目标实力强
									desc = img..player.Name.."的境界与"..target.Name.."相差甚远，但谁让"..player.Name.."女修呢，女性在大部分时候都要更占便宜一点，"..player.Name.."刚提出双修，对方便同意了，随后便是数个时辰的疯狂，"..player.Name.."甚至感觉自己被操的快断片了，数次的昏死过去在被活活操醒，让"..player.Name.."了解到高位修士的持久力是多么“恐怖如斯”，不过这一切都是值得的，随着对方虎躯颤了那么几颤，"..player.Name.."只觉一股滚烫且极具生命活力的精元灌注进自己的身体……"
									else--女主，和男目标一样强
									desc = img..player.Name.."的境界与"..target.Name.."相差无几，开口提出双修对方也没有拒绝。\n这是一次成功的双修。"
									end
								end
		...
		if tsCD == 0 then
		desc = img..player.Name.."刚双修完毕，体内阴阳二气无比和谐，暂时无法双修。"
		else
		desc = img..target.Name.."刚双修完毕，体内阴阳二气无比和谐，暂时无法双修。"
		end
	end
	return desc;
end
function tbMagic:EnterHappy(npc)
	...
	function(Key)
		...
		local key = Key + 1;
		local win = GameMain:GetMod("Windows"):GetWindow("SampleWindow");
		local target = ThingMgr:FindThingByID(npc.ID)
		local player = ThingMgr:FindThingByID(self.bind.ID)
		local random = player.LuaHelper:RandomInt(1,100);
		local random_pic = player.LuaHelper:RandomInt(214,320);
		local pic = "Spr/preview/"..random_pic..".png"
		local img = "[img]"..pic.."[/img]\n"
		local desc = ""
		...
		elseif key == 3 then
			...
			win:SetPicture(pic);
			win:SetText(desc);
			--local desc = tbMagic:ShuangxiuStart(npc1, npc2)
			return desc
			--CS.Wnd_JianghuTalk.Instance:Talk(npc1,npc2)
		elseif key == 4 then
			desc = self:Action(player, target);
			win:SetPicture(pic);
			win:SetText(desc);
			return desc;
		elseif key == 5 then
			desc = self:Action(target, player);
			win:SetPicture(pic);
			win:SetText(desc);
			return desc;
		else
	...
	--if sw == 1 then 
	--	Hudong:ShuangxiuStart(npc1, npc2)
	--end
