...
local tbMagic = tbTable:GetMagic("HappyGet");
...
function tbMagic:MagicEnter(IDs, IsThing)
	...
	if self:GetLevel(target) <= self:GetLevel(self.bind) then
		if target.PropertyMgr:FindModifier("HappyGetDebuff") == nil then
			if self.bind.LuaHelper:IsLearnedEsoteric("Gong_HappyMan_5") == true or target.Sex ~= self.bind.Sex then
